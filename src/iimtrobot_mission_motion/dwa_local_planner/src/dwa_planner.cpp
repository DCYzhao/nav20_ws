...
void DWAPlanner::reconfigure(DWAPlannerConfig &config) {
  ...
  if (vx_samp <= 0) {
    ROS_WARN(
        "You've specified that you don't want any samples in the x dimension. We'll at least assume that you "
        "want to sample one value... so we're going to set vx_samples to 1 instead");
    vx_samp = 1;
  ...
  if (vy_samp <= 0) {
    ROS_WARN(
        "You've specified that you don't want any samples in the y dimension. We'll at least assume that you "
        "want to sample one value... so we're going to set vy_samples to 1 instead");
    vy_samp = 1;
  ...
  if (vth_samp <= 0) {
    ROS_WARN(
        "You've specified that you don't want any samples in the th dimension. We'll at least assume that "
        "you want to sample one value... so we're going to set vth_samples to 1 instead");
    vth_samp = 1;
...
DWAPlanner::DWAPlanner(std::string name, base_local_planner::LocalPlannerUtil *planner_util)
  ...
  map_viz_.initialize(
      name, planner_util->getGlobalFrame(),
  ...
  critics.push_back(
      &goal_front_costs_);  // prefers trajectories that make the nose go towards (local) nose goal
  critics.push_back(&alignment_costs_);  // prefers trajectories that keep the robot nose on nose path
  critics.push_back(&path_costs_);       // prefers trajectories on global path
  critics.push_back(
      &goal_costs_);  // prefers trajectories that go towards (local) goal, based on wave propagation
...
bool DWAPlanner::getCellCosts(int cx, int cy, float &path_cost, float &goal_cost, float &occ_cost,
                              float &total_cost) {
...
base_local_planner::Trajectory DWAPlanner::findBestPath(const geometry_msgs::PoseStamped &global_pose,
  ...
  if (publish_traj_pc_) {
    ...
    cloud_mod.setPointCloud2Fields(5, "x", 1, sensor_msgs::PointField::FLOAT32, "y", 1,
                                   ...
                                   "theta", 1, sensor_msgs::PointField::FLOAT32, "cost", 1,
                                   sensor_msgs::PointField::FLOAT32);

    unsigned int num_points = 0;
    for (std::vector<base_local_planner::Trajectory>::iterator t = all_explored.begin();
         t != all_explored.end(); ++t) {
  ...
  oscillation_costs_.updateOscillationFlags(pos, &result_traj_,
